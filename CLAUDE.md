...
| `tabBarStyle` 只設 padding 不設 height，或設了任意 height 不配套 padding | 框架內部固定 height=49+insets.bottom，自訂 padding 會吃掉內容空間；正確公式：`height: 56 + bottomInset, paddingBottom: bottomInset`，確保內容空間永遠 49px（UIKit 標準） |
