...
| C-22 | `mobile/app/(tabs)/_layout.tsx` 的 tab bar 必須包含 `Platform.OS === "web"` 的 `Math.max` bottomInset fallback，且 `tabBarStyle` 必須同時設定 `height: 56 + bottomInset` 和 `paddingBottom: bottomInset`（配套公式，確保內容空間永遠 49px） | SLA-2 | 2026-04-05 tab bar 文字被裁切（同一 bug 第 5 次） |
