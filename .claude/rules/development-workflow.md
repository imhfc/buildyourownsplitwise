...
## 制定規則前必須驗證技術依據（2026-04-06 回顧）

- 錯誤的規則比沒有規則更危險——遵循錯誤規則導致的 bug 比無規則更難除錯，因為開發者相信自己「做對了」
- 典型案例：「禁止固定 height」規則制定時未查 React Navigation `getTabBarHeight()` 實作，遵循規則的修改反而引入 bug，且此 bug 第 5 次復發
- 制定任何「禁止 X」或「必須 Y」的規則，必須同時記錄框架底層行為作為依據（如：React Navigation 沒有自訂 height 時固定回傳 `49 + insets.bottom`，自訂 padding 會吃掉這 49px 內容空間）
- 若無法說明「為何 X 有害的底層原因」，禁止制定禁止 X 的規則
