...
Logo 應在 app 的多個接觸點出現，但以**克制低調**的方式呈現。

| 位置 | 尺寸 | 樣式 | 說明 |
|------|------|------|------|
| 登入頁 | 120px | 原色 | 主視覺，搭配入場動畫 |
| 首頁 header | 28px | 原色 / dark tintColor | 取代純文字標題，每次開 app 第一眼 |
| 空狀態 | 56px | `opacity-20` | 搭配功能 icon，填補空白頁面 |
| 帳號頁底部 | 32px | `opacity-30` | 低調 watermark |
| Splash screen | 系統預設 | 原色 | app 啟動畫面 |
| App icon | 系統預設 | 原色 | 手機桌面圖示 |

### 規則

- Logo 資源使用 `logo-transparent.png`（透明底），**禁止**用 `icon.png`（白底）
- Dark mode 使用 `tintColor: "#E2E8F0"` 染色
- 非主視覺位置的 logo 必須降低 opacity（20-30%），避免喧賓奪主
- Logo 不加圓角、不加邊框、不加陰影
